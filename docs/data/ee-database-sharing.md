...
To not share a map anymore hit `UNLINK` within the `Sharing` info line of a set's details. This will not remove the latest version from anyone who added that map but it would not get updated for them anymore. If removed by the ones using it it cannot be added back.
#### Map ownership
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIzMTQwNTIzOV19
-->